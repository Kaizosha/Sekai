...
vertex SekaiPointOut sekaiUnifiedParticleVertex(
    ...
    float angle = uniforms.elapsed * uniforms.autoRotationSpeed;
    float4 spin = float4(0.0, sin(angle * 0.5), 0.0, cos(angle * 0.5));
    float3 position = rotateQuaternion(rotateQuaternion(particles[vertexID].position, spin), uniforms.quaternion);
    ...
    if (!visible) output.position = float4(2.0, 2.0, 1.0, 1.0);
    output.color = uniforms.color;
    output.depth = position.z;
    output.optical = uniforms.optical;
    float daylight = smoothstep(-0.18, 0.18, dot(position, uniforms.environment.xyz));
...
fragment float4 sekaiUnifiedLineFragment(SekaiPointOut input [[stage_in]]) {
    ...
    return float4(input.color.rgb * 1.08 * input.illumination, input.color.a * depthFade);
}
